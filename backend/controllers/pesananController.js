...
const getAllLokasi = async (req, res) => {
  try {
    const [lokasi] = await db.query('SELECT lokasiId as id, name FROM Lokasi')
    res.json(lokasi)
  } catch (err) {
...
const getAllProduk = async (req, res) => {
  try {
    ...
    // 🛑 LOGGING BARU UNTUK DIAGNOSIS ERROR 500
    console.log('----------------------------------------------------')
  ...
  try {
    const [pesanan] = await db.query(
      `SELECT
         ...
         buktiPembayaranUrl, tipePesanan // ✅ TAMBAH: tipePesanan
       FROM Pemesanan
...
const getPesananById = async (req, res) => {
  ...
  try {
    const [pesananResult] = await db.query('SELECT * FROM Pemesanan WHERE pesananId = ?', [id])
...
const createPesanan = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ success: false, message: 'Format data items (JSON string) tidak valid.' })
  }
  ...
  try {
    ...
    const queryPemesanan = `
      INSERT INTO Pemesanan (
        ...
        totalHarga, alamatPengiriman, kontakPelanggan, buktiPembayaranUrl, tipePesanan // ✅ TAMBAH KOLOM
      )
      VALUES (?, ?, NOW(), ?, ?, ?, ?, NULL, ?) // ✅ TAMBAH PLACEHOLDER
    `
    ...
    await connection.query('UPDATE Pemesanan SET buktiPembayaranUrl = ? WHERE pesananId = ?', [
      cdnUrl,
      newPesananId,
    ])
    ...
    res
      .status(500)
      .json({ success: false, message: 'Error server saat membuat pesanan', error: err.message })
  } finally {
    if (connection) {
      connection.release()
    }
...
const createPesananOffline = async (req, res) => {
  ...
  const statusAwal = 'Perlu Validasi'
  const tipePesanan = 'Offline Report'
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: 'Error server saat membuat pesanan offline',
        error: err.message,
      })
  } finally {
    if (connection) {
...
const updateLokasiPesanan = async (req, res) => {
  ...
  try {
    const [updateResult] = await db.query('UPDATE Pemesanan SET lokasiId = ? WHERE pesananId = ?', [
      lokasiId,
      id,
    ])
