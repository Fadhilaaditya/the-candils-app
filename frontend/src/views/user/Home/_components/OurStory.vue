<template>
  <section class="bg-white py-16 h-[443px]">
    ...
    >
      ...
      <p class="mt-6 text-3xl leading-7 text-neutral-700 md:text-3lg">{{ content.quote }}</p>
      <div class="mt-8">
        <a
          :href="content.cta.href"
          class="rounded-md text-white bg-[#68663F] px-4 py-2 text-sm hover:bg-[#7a784a]"
          >{{ content.cta.label }}</a
        >
      </div>
    </div>
...
import fallbackData from './data/ourstory.json'
...
const props = defineProps<{ endpoint?: string; initialData?: OurStoryData }>()

const content = ref<OurStoryData>({ ...fallbackData })
