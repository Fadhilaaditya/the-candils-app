<template>
  <div
    class="fixed inset-0 z-40 bg-black bg-opacity-60 backdrop-blur-sm overflow-y-auto h-full w-full transition-all duration-300"
    @click.self="$emit('close')"
  >
    <div
      class="relative top-10 mx-auto p-6 w-full max-w-4xl shadow-2xl rounded-2xl bg-white mb-10 transform transition-all duration-300"
    >
      ...
      <form @submit.prevent="submitPesanan" class="mt-6 space-y-6">
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label for="namaPelanggan" class="block text-sm font-semibold text-gray-700 mb-2"
            ...
            <div>
              <label for="kontakPelanggan" class="block text-sm font-semibold text-gray-700 mb-2"
                >Kontak Pelanggan *</label
              >
              <input
                type="text"
                v-model="form.kontakPelanggan"
                id="kontakPelanggan"
                required
          ...
          <div class="mt-4">
            <label for="alamatPengiriman" class="block text-sm font-semibold text-gray-700 mb-2"
              >Alamat Pengiriman *</label
            >
            <textarea
              v-model="form.alamatPengiriman"
              id="alamatPengiriman"
              rows="2"
              required
          ...
          <div class="mt-4">
            <label for="lokasiId" class="block text-sm font-semibold text-gray-700 mb-2"
              >Lokasi Penjualan *</label
            >
            <select
              v-model="form.lokasiId"
              id="lokasiId"
              required
            ...
            >
              ...
              <option
                v-for="lokasi in props.lokasiList"
                :key="lokasi.id || lokasi.lokasiId"
                :value="lokasi.id || lokasi.lokasiId"
              >
        ...
        <div
          class="bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-100 p-5 rounded-xl"
        >
            ...
            >
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-2">
                  <span
                    class="bg-gradient-to-r from-purple-500 to-pink-500 text-white w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold"
                    >{{ index + 1 }}</span
                  >
              ...
              <div class="grid grid-cols-12 gap-3">
                ...
                <div class="col-span-4">
                  ...
                  >
                    ...
                    <option
                      v-for="ukuran in availableUkuran(item.produkId)"
                      :key="ukuran.ukuranId"
                      :value="ukuran.ukuranId"
                    >
                      {{ ukuran.namaUkuran }}
                    </option>
                  </select>
...
// Tipe Lokal (Disesuaikan agar sesuai dengan response API yang sudah diperbaiki)
interface ItemInput {
...
// ✅ FIX: Menggunakan 'harga' (sesuai alias SQL) bukan 'hargaUnit'
interface Produk {
  produkId: number
  namaProduk: string
  harga: number
...
const form = ref({
  ...
  items: [{ produkId: null, ukuranId: null, quantity: 1, subtotal: 0 }] as ItemInput[],
})
...
const updateItemPrice = (item: ItemInput) => {
  ...
  if (produk) {
    // ✅ FIX: Menggunakan produk.harga (dari alias SQL)
...
const loadMasterData = async () => {
  try {
    ...
    const [produkRes, ukuranRes] = await Promise.all([
      getAllProduk(), // Akan mengembalikan field 'harga' karena perbaikan di controller
      getAllUkuran(),
    ])

    // 2. MAPPING PRODUK
    produkList.value = produkRes.data
      .map((p: { produkId?: number; namaProduk?: string; harga?: number; hargaUnit?: number }) => ({
        produkId: Number(p.produkId),
        namaProduk: p.namaProduk || '',
        // ✅ FIX: Memprioritaskan field 'harga' dari response API yang sudah diperbaiki
        harga: Number(p.harga || p.hargaUnit || 0),
      }))
    ...
    if (produkList.value.length === 0) {
      // Alert ini adalah respons terhadap error 500 yang mungkin terjadi saat fetching
    ...
    // Alert yang muncul dari status code 500 di backend
    const errorMessage =
