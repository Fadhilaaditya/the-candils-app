      ...
      <table class="w-full">
        ...
        <tbody>
          ...
          >
            ...
            <!-- ✅ PERUBAHAN: Langsung pakai variant.foto (Cloudinary URL) -->
            <td class="px-4 py-3">
              <img
...
// ✅ NEW: Handle image URL
const getImageUrl = (fotoUrl: string | null | undefined): string => {
  if (!fotoUrl) {
    return 'https://placehold.co/64x64/eee/ccc?text=No+Image'
  }
  
  // Cloudinary URL sudah lengkap, langsung return
  if (fotoUrl.startsWith('http://') || fotoUrl.startsWith('https://')) {
    return fotoUrl
  }
  
  // Fallback untuk data lama (kalau ada yang masih path lokal)
  return 'https://placehold.co/64x64/eee/ccc?text=No+Image'
}
